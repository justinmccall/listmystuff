...
The following items available for sale:
<br>
<?php echo str_replace("{{b}}","",str_replace("{{bl}}","<br>- ",$newContents)); ?><br>
<br>
While the following items have been reserved, but not yet taken.  If you are interested, let me know and I will add you to the reservation list.
<br>
<?php echo str_replace("{{b}}","",str_replace("{{bl}}","<br>- ",$resContents)); ?><br>
<br>
These items are no longer available for sale:
<br>
<?php echo str_replace("{{b}}","",str_replace("{{bl}}","<br>- ",$takenContents)); ?><br>
<br>
Click the link below to view the items:<br>
https://mystuff.resonate.co.za/users/<?php echo $uri[2]; ?>/
</div>
...
while ($row = $result->fetch_assoc()) {
    ...
    switch($row['status']){
        ...
        case "Reserved":
            ...
            $bgColor = "#efefef";
            $fontColor = "rgba(0,0,0,.81) !important";
            $statusLabel = '<label class="ui green ribbon label">RESERVED</label>';
            break;
        case "Taken":
            $borderColor = "#D20808";
            $opacity = "0.5";
            $fontColor = "#ffffff !important";
            $statusLabel = '<label class="ui red ribbon label">TAKEN</label>';
    ...
    while ($img = $images->fetch_assoc()) {
        $theImage = $img['image_path'];
    }    

    ?>

<!--     <div class="ui grid listing-mobile" style="background-color: #ffffff; border-radius: 10px; margin: 15px 0; opacity: <?php echo $opacity; ?>; overflow: hidden;">
        <div class="four wide column" style="background-image: url('/<?php echo $theImage; ?>'); background-size: cover; background-position: center;"></div>
        <div class="twelve wide column" style="padding-bottom: 20px;">
            <?php echo $statusLabel; ?>
            <div class="header" style="margin-top: 10px; font-weight: light;"><?php echo $row['title']; ?></div>
            <div class="description" style="display: none;"><?php echo nl2br($row['description']); ?></div>            
        </div>
    </div>
 -->
    <?php
    ...
    echo '
      <div class="ui '.$disableClass.' card listing-desktop">
